...
GitHub：`bitqs/thegametheory`（结局页有作者署名链接）。线上 https://thegametheory.pages.dev。
...
- 牌面=游戏设计原则+一句招供（words 22 常规/wordsRare 8 深层）；`drawPrinciple` 按 BEATMATCH
  与当前 beat 呼应；同局词/画作不重复（usedW/usedWR/usedArt）。收藏=集齐设计原则。
- 已装心理学武器：禀赋进度（预填 2 格+climax 点名）/目标梯度（≥70% gg 加速）/新起点（升级 quip）/
  好奇缺口（故事拍牌背露半句）/near-miss 神经实锤（Clark 2009）/新元素 .newin 金光脉冲。
...
- 手机端验收用 `?v=N` 绕 Safari 缓存；本地 python 服 iCloud 目录会间歇 404，验证优先打线上 preview。
