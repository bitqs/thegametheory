...
- 调研三件套：`docs/meta-games-research.md`（元游戏点子）/ `docs/copy-constitution.md`（**改文案先读**）/
  `docs/gamification-audit.md`（游戏化审计+TOP12+红线：机制命名必须准、暗模式必须自我点名）。
- 牌面=游戏设计原则+一句招供（i18n words 22 常规/wordsRare 8 深层）；`drawPrinciple` 按 BEATMATCH
  与当前 beat 呼应，同局词/画作均不重复（state.usedW/usedWR/usedArt）。
- 已装心理学武器：禀赋进度（doneActions 预填 2，climax 点名）/目标梯度（≥70% gg 加速）/
  新起点（升级 quip）/好奇缺口（故事拍牌背露半句）/near-miss 神经实锤（Clark 2009）。
